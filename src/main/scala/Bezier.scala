...
object Bezier {
    ...
    def append(bps: BezierAsPoints, knot: Point): Unit = {
        val (ks, c1s, c2s, closed) = bps
        ks += knot
        (ks.length, closed) match {
            case (1, _) => 
            case (2, _) => 
            ...
            case (n, true) =>
                ...
                Range(0, n).foreach({ i =>
                    ...
                    rhs(i) = ks(i).x * 4 + ks(j).x * 2
                })
                val xs = solveCyclic(a, b, c, 1, 1, rhs)

                Range(0, n).foreach({ i =>
                    val j = if (i == n-1) 0 else i+1
                    rhs(i) = ks(i).y * 4 + ks(j).y * 2
                })
                ...
                Range(0, n).foreach({ i =>
                    c1s += Point(xs(i), ys(i))
                    c2s += Point(ks(i).x * 2 - xs(i), ks(i).y * 2 - ys(i))
                })
        } 
    }
    ...
    private def solveTridiagonal(a: ArrayBuffer[Double], b: ArrayBuffer[Double], c: ArrayBuffer[Double], rhs: ArrayBudder[Double]): ArrayBuffer[Double] = {
        val n = rhs.length
